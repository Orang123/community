...
public class ElasticsearchTests {
    ...
    public void testSearchByRepository() {
        ...
        // 底层获取得到了高亮显示的值, 但是没有返回,要高亮需要用elasticTemplate中实现的方法
    ...
    public void testSearchByTemplate() {
        SearchQuery searchQuery = new NativeSearchQueryBuilder()
                ...
                .withSort(SortBuilders.fieldSort("score").order(SortOrder.DESC))
                .withSort(SortBuilders.fieldSort("createTime").order(SortOrder.DESC))
                .withPageable(PageRequest.of(0,10))//page当前页编号设置为11(实际第12页,第一页编号从0开始), 实际页面编号为11的数据只有1条数据
                .withHighlightFields(
                        new HighlightBuilder.Field("title").preTags("<em>").postTags("</em>"),
                        new HighlightBuilder.Field("content").preTags("<em>").postTags("</em>")
                ).build();
        ...
        //总共111条数据 每页10条数据,共12页.
        System.out.println(page.getTotalElements());//所有数据个数
        System.out.println(page.getTotalPages());//总页数
        System.out.println(page.getNumber());//当前页的编号 页的编号从0开始
        System.out.println(page.getSize());//分页的每页的数据个数
        for(DiscussPost post : page) {
            System.out.println(post);
        }
    }
    ...
    public void insertAllPosts() {
        for(int i=109;i<=293;i++) {
            DiscussPost post = discussMapper.selectDiscussPostById(i);
            if(post != null)
                discussRepository.save(post);
        }
    }
